...
# **** THIS VERSION WORKS where a single partition is run on each node.
...
mpiexec -n 17 --mca btl_tcp_if_include eth0  --allow-run-as-root \
 --hostfile $1  --report-bindings \
 python3.8 /competition/replace_solver.py /competition/opensmt-splitter /competition/cvc5 $2 $1
